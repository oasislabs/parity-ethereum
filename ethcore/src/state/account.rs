...
pub struct Account {
	...
	original_storage_cache: Option<(H256, RefCell<LruCache<H256, Vec<u8>>>)>,
...
impl Account {
	#[cfg(test)]
	/// General constructor.
	pub fn new(balance: U256, nonce: U256, storage: HashMap<H256, Vec<u8>>, code: Bytes) -> Account {
		Account {
			balance: balance,
			nonce: nonce,
	...
	pub fn storage_at(&self, db: &HashDB<KeccakHasher, DBValue>, key: &H256) -> TrieResult<Option<Vec<u8>>> {
	...
	pub fn original_storage_at(&self, db: &HashDB<KeccakHasher, DBValue>, key: &H256) -> TrieResult<Option<Vec<u8>>> {
	...
	fn get_and_cache_storage(storage_root: &H256, storage_cache: &mut LruCache<H256, Vec<u8>>, db: &HashDB<KeccakHasher, DBValue>, key: &H256) -> TrieResult<Option<Vec<u8>>> {
	...
	pub fn cached_original_storage_at(&self, key: &H256) -> Option<Vec<u8>> {
		match &self.original_storage_cache {
	...
	fn cached_moved_original_storage_at(&self, key: &H256) -> Option<Vec<u8>> {
...
impl Account {
	...
	pub fn prove_storage(&self, db: &HashDB<KeccakHasher, DBValue>, storage_key: H256) -> TrieResult<(Vec<Bytes>, Vec<u8>)> {
...
mod tests {
	...
	fn storage_at() {
		...
		assert_eq!(a.storage_at(&db.immutable(), &0x00u64.into()).unwrap(), Some(H256::from(0x1234u64).to_vec()));

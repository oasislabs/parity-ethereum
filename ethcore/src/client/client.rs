...
mod tests {
	...
	fn should_return_block_receipts() {
		...
		assert_eq!(receipts[0].cumulative_gas_used, 34_100.into());
		assert_eq!(receipts[0].gas_used, 34_100.into());

		assert_eq!(receipts[1].transaction_index, 1);
		assert_eq!(receipts[1].block_number, 2);
		assert_eq!(receipts[1].cumulative_gas_used, 68_200.into());
		assert_eq!(receipts[1].gas_used, 34_100.into());

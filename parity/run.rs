...
fn prepare_account_provider(spec: &SpecType, dirs: &Directories, data_dir: &str, cfg: AccountsConfig, passwords: &[Password]) -> Result<AccountProvider, String> {
	...
	let account_settings = AccountProviderSettings {
		...
		blacklisted_accounts: 	match *spec {
			SpecType::Morden | SpecType::Ropsten | SpecType::Kovan | SpecType::Sokol | SpecType::Dev | SpecType::Oasis => vec![],
			_ => vec![
...
fn wait_for_drop<T>(w: Weak<T>) {
	...
	// workaround for hang on shutdown after connecting MetaMask
	// possible related issue: https://github.com/paritytech/parity-ethereum/issues/9807
